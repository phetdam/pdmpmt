...
/**
 * Traits helper providing conditions for the `write` range type.
 *
 * @tparam T type
 */
template <typename T, typename = void>
struct is_write_input_range : std::false_type {};

/**
 * True specialization for a range with an `is_write_input_iterator` iterator.
 *
 * @tparam T type
 */
template <typename T>
struct is_write_input_range<
  T,
  std::enable_if_t<
    is_write_input_iterator<decltype(std::begin(std::declval<T>()))>::value &&
    is_write_input_iterator<decltype(std::end(std::declval<T>()))>::value
  >
> : std::true_type {};
 ...
 */
template <typename T>
using write_input_range_t = std::enable_if_t<is_write_input_range<T>::value>;

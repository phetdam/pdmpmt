 ...
 * @file cpu_set.hh
 * @author Derek Huabg
 * @brief C++ header for a logical CPU set representing a CPU affinity mask
 * @copyright MIT License
 */

#ifndef PDMPMT_CPU_SET_HH_
#define PDMPMT_CPU_SET_HH_
 ...
 * @param cpus CPU set
 */
inline auto& operator<<(std::ostream& out, const cpu_set& cpus)
{
 ...
 */
inline auto operator<<(std::ostream& out, const cpu_set::text_format& fmt)
{
 ...
 * @param cpus CPU set
 */
inline auto& operator<<(const cpu_set::text_formatter& out, const cpu_set& cpus)
{
  // TODO: no range-for support yet
  out.out() << "[";
  for (auto i = 0u; i < cpus.size(); i++)
